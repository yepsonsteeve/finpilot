...
const INVESTMENTS = [
  ...
  { name: "Epargne",     value: 6500,  color: "#a78bfa", icon: "🏦" },
];
const CASHFLOW = [
  { month: "Jan", revenus: 4200, depenses: 2800 },
  { month: "Fev", revenus: 4200, depenses: 3100 },
...
const CHARGES = [
  ...
  { label: "Telephone",      amount: 30,  due: "20 Mai", icon: "📱", status: "upcoming" },
  { label: "Internet",       amount: 35,  due: "22 Mai", icon: "🌐", status: "upcoming" },
  { label: "Electricite",    amount: 120, due: "28 Mai", icon: "⚡", status: "pending"  },
  { label: "Mutuelle sante", amount: 75,  due: "01 Jun", icon: "❤️", status: "future"   },
];
const CAT_COLORS = { Logement:"#6c8eff", Alimentation:"#00e5a0", Loisirs:"#ff6c8e", Transport:"#ffd700", Sante:"#a78bfa" };
const ICONS_MAP = { Logement:"🏠", Alimentation:"🛒", Loisirs:"🎮", Transport:"🚗", Sante:"💊" };
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16,paddingBottom:16}}>
      <div style={{...card,background:"linear-gradient(135deg,rgba(0,229,160,0.12),rgba(108,142,255,0.12))",textAlign:"center",padding:"28px 20px"}}>
        <div style={{fontSize:12,color:"#888",letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>Solde disponible</div>
        <div style={{fontSize:44,fontWeight:800,color:"#fff",letterSpacing:-2}}>{balance.toLocaleString()} EUR</div>
        <div style={{display:"flex",justifyContent:"center",gap:24,marginTop:16}}>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:11,color:"#00e5a0"}}>REVENUS</div>
            <div style={{fontSize:18,fontWeight:700,color:"#fff"}}>{income.toLocaleString()} EUR</div>
          </div>
          <div style={{width:1,background:"rgba(255,255,255,0.1)"}}/>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:11,color:"#ff6c8e"}}>DEPENSES</div>
            <div style={{fontSize:18,fontWeight:700,color:"#fff"}}>{totalExp.toLocaleString()} EUR</div>
          </div>
          <div style={{width:1,background:"rgba(255,255,255,0.1)"}}/>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:11,color:"#ffd700"}}>PATRIMOINE</div>
            <div style={{fontSize:18,fontWeight:700,color:"#fff"}}>{(totalInvest/1000).toFixed(0)}k EUR</div>
          </div>
        </div>
      </div>
      <div style={{...card}}>
        ...
        <div style={{fontSize:12,color:"#555",marginTop:6}}>Tres bonne gestion · +3 pts ce mois</div>
      </div>
      <div style={{...card}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14,paddingBottom:16}}>
      ...
      {showForm && (
        <div style={{...card,display:"flex",flexDirection:"column",gap:10}}>
          ...
          <input type="number" placeholder="Montant" value={form.amount} onChange={e=>setForm(p=>({...p,amount:e.target.value}))} style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"12px 14px",color:"#fff",fontSize:14,outline:"none",width:"100%"}}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {Object.entries(byCategory).map(([cat,amt]) => (
          <div key={cat} style={{...card,padding:"14px 16px"}}>
            <div style={{fontSize:11,color:"#666",marginBottom:4}}>{cat.toUpperCase()}</div>
            ...
            <div style={{height:3,background:"rgba(255,255,255,0.07)",borderRadius:2,marginTop:8}}>
              <div style={{width:`${Math.min(100,(amt/total)*100*2)}%`,height:"100%",background:CAT_COLORS[cat]||"#fff",borderRadius:2}}/>
            </div>
          </div>
        ))}
      </div>
        ...
        {expenses.slice().reverse().map(e => (
          <div key={e.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{fontSize:14,color:"#ff6c8e",fontWeight:700}}>-{e.amount} EUR</div>
              <button onClick={() => setExpenses(p=>p.filter(x=>x.id!==e.id))} style={{background:"rgba(255,100,100,0.1)",border:"none",borderRadius:8,width:28,height:28,color:"#ff6c8e",cursor:"pointer",fontSize:14}}>x</button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14,paddingBottom:16}}>
      <div>
        ...
        <div style={{fontSize:12,color:"#00e5a0"}}>Patrimoine : {total.toLocaleString()} EUR</div>
      </div>
      <div style={{...card}}>
        ...
        return (
          <div key={i} style={{...card,display:"flex",alignItems:"center",gap:14}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:12,color:"#555",marginTop:2}}>Echeance : {c.due}</div>
            </div>
...
function Conseils({ expenses }) {
  ...
  if (ratio <= 70) tips.push({icon:"✅", text:`Excellent ! Tu depenses ${ratio}% de tes revenus.`, color:"#00e5a0"});
  tips.push({icon:"💡", text:"Regle 50/30/20 : 50% besoins, 30% envies, 20% epargne.", color:"#6c8eff"});
  tips.push({icon:"📅", text:"Verifie tes abonnements chaque trimestre.", color:"#a78bfa"});
  const claudePrompt = encodeURIComponent(`Bonjour ! Revenus: ${income}EUR, Depenses: ${total}EUR, Solde: ${income-total}EUR. Aide-moi a optimiser mon budget.`);
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16,paddingBottom:16}}>
      <div>
        <div style={{fontSize:22,fontWeight:800,color:"#fff"}}>Conseils</div>
        <div style={{fontSize:12,color:"#555"}}>Analyse automatique</div>
      </div>
      <div style={{...card}}>
      ...
      <div style={{...card,textAlign:"center",padding:"28px 20px"}}>
        ...
        <div style={{fontSize:17,fontWeight:800,color:"#fff",marginBottom:8}}>Conseil personnalise ?</div>
        <div style={{fontSize:13,color:"#666",marginBottom:20}}>Ouvre Claude.ai avec tes donnees.</div>
